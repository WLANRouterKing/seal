...
export const useMessageStore = create<MessageState>((set, get) => ({
  ...
  initialize: async (_userPubkey: string, userPrivateKey: string) => {
    ...
    try {
      ...
      decryptedMessages.forEach((msg) => {
        ...
        messageMap.set(
          contactPubkey,
          [...existing, msg].sort((a, b) => a.createdAt - b.createdAt)
        )
      })
  ...
  sendFileMessage: async (
    ...
    try {
      ...
      const selfContent = caption ? `${caption}\n[file:${fileData}]` : `[file:${fileData}]`
      ...
      const finalContent = caption ? `${caption}\n[file:${finalFileData}]` : `[file:${finalFileData}]`
      ...
      async (event) => {
        ...
        try {
          ...
          messages.set(
            ...
            [...contactMessages, message].sort((a, b) => a.createdAt - b.createdAt)
          )
          set({ messages: new Map(messages) })
          ...
          if (initialSyncDone && get().activeChat !== unwrapped.senderPubkey) {
            ...
            await notificationService.showMessageNotification(senderName, unwrapped.senderPubkey)
          }
        } catch (error) {
  ...
  markAsRead: async (pubkey: string) => {
    ...
    const chats = get().chats.map((c) => (c.pubkey === pubkey ? { ...c, unreadCount: 0 } : c))
    set({ chats })
    ...
    try {
      ...
      if (filteredMessages.length === 0) {
        ...
        const chats = get().chats.map((c) =>
          c.pubkey === contactPubkey ? { ...c, lastMessage, updatedAt: lastMessage.createdAt } : c
        )
