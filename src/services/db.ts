...
interface SealChatDB extends DBSchema {
    [STORES.KEYS]: {
        key: string
...
let dbPromise: Promise<IDBPDatabase<SealChatDB>> | null = null
    ...
    if (!dbPromise) {
        dbPromise = openDB<SealChatDB>(DB_NAME, DB_VERSION, {
            upgrade(db, oldVersion, newVersion) {
                ...
                // Version 2: Add blocked contacts
                if (oldVersion < 2) {
                    db.createObjectStore(STORES.BLOCKED_CONTACTS, {keyPath: 'pubkey'})
                }
...
export async function saveBlockedContact(contact: Contact): Promise<void> {
    const db = await getDB()

    if (isEncryptionUnlocked()) {
        const envelope = await encryptForStorage(contact)
        await db.put(STORES.BLOCKED_CONTACTS, {...envelope, pubkey: contact.pubkey} as unknown as Contact)
    } else {
        await db.put(STORES.BLOCKED_CONTACTS, contact)
    }
}

async function decryptBlockedContact(contact: unknown): Promise<Contact | null> {
    if (isEncryptedContact(contact)) {
        return decryptFromStorage<Contact>(contact)
    }
    return contact as Contact
...
export async function getBlockedContact(pubkey: string): Promise<Contact | undefined> {
    const db = await getDB()
    const blockedContact = await db.get(STORES.BLOCKED_CONTACTS, pubkey)
    if (!blockedContact) return undefined

    const decrypted = await decryptBlockedContact(blockedContact)
    return decrypted ?? undefined
}

export async function getAllBlockedContacts(): Promise<Contact[]> {
    const db = await getDB()
    const blockedContacts = await db.getAll(STORES.BLOCKED_CONTACTS)
    const decrypted = await Promise.all(blockedContacts.map(decryptContact))
    return decrypted.filter((c): c is Contact => c !== null)
}

export async function deleteBlockedContact(pubkey: string): Promise<void> {
    const db = await getDB()
    await db.delete(STORES.BLOCKED_CONTACTS, pubkey)
}
...
export async function deleteRelay(url: string): Promise<void> {
    ...
    if (isEncryptionUnlocked()) {
        ...
        try {
            await db.delete(STORES.RELAYS, urlHash)
        } catch { /* ignore */
        }
    }
}
...
export async function clearAllData(): Promise<void> {
    ...
    const tx = db.transaction(
        [STORES.KEYS, STORES.MESSAGES, STORES.CONTACTS, STORES.BLOCKED_CONTACTS, STORES.SETTINGS, STORES.RELAYS, STORES.DELETED_MESSAGES],
        'readwrite'
    )
    await Promise.all([
        ...
        tx.objectStore(STORES.BLOCKED_CONTACTS).clear(),
